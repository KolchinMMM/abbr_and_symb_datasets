...
model = AutoModelForSeq2SeqLM.from_pretrained('ai-forever/FRED-T5-1.7B')

directory_datasets = "datasets/symbols"


def preprocess(examples):
    model_inputs = tokenizer(examples['question'], max_length=200)
    with tokenizer.as_target_tokenizer():
        labels = tokenizer(examples['answer'], max_length=200)
...
for i in trange(len(file.values)):
    ...
    dict_correct = merge_dicts(dict_correct, d_ans)
